...
3. 记录预处理、伪迹处理、试次数/排除、ROI 预设或全脑探索、多重比较、时间窗/频带选择、特征工程、交叉验证和数据泄漏风险。
4. 评价方法与机制问题的匹配度，区分可检测到指标与能识别目标机制；检查反向推断和跨模态尺度错配。
5. 对每个新增方法建议记录“具体失败—失败后不同动作—最终裁决”；不能改变机制判断的复杂方法不新增。
6. 标记报告不充分、预处理不透明、分析自由度和统计风险，输出方法评价和降级后的允许措辞。
...
- 报告是否透明？
- 是否区分行为、主观、神经和生理证据层级？
- 是否核对 ROI/全脑、多重比较、分析灵活性、交叉验证和反向推断？

...
- 运行模式：`standard`
- 默认综述类型：混合型机制综述
- 可升级方向：如果检索和筛查条件满足，再升级为范围综述或系统综述；不要默认声称 systematic review。
- 主题包：必须加载 `topic_packs/rem_emotional_memory/`
- 目标格式：中文核心综述风格优先，同时保留外文 Q1 review 预备稿结构。
...
8. `07_prewrite`：冻结综述方法段、章节论证路径、允许/禁止的结论、图表计划、正文格式和段落 ID 策略。
9. `08_manuscript`：完成 Markdown 综述草稿和 Word 草稿。
10. `09_audit`：执行段落—来源对应矩阵和参考文献一致性核查。

## 数据库范围

请覆盖以下数据库或资料来源：PubMed、Web of Science、PsycINFO、Scopus、CNKI、万方、维普、Google Scholar、Semantic Scholar、Crossref、Zotero 本地库。所有检索式、检索日期、筛选条件、结果数量、导出数量都必须记录。无法访问的数据库必须记录原因，不得假装已经检索。
...
- 心理学和认知神经科学研究先用自定义框架评价样本、设计、测量、神经指标、统计报告和结论适用性。
- 适合正式工具时，按研究类型选择 MMAT、JBI、RoB 2、ROBINS-I、SYRCLE 或 AMSTAR 2。
- 不使用正式工具时，必须记录原因。
...
- 不要编造文献、DOI、PMID、CNKI ID、万方 ID、维普 ID、样本量或研究结论。
- 不要把综述文献当成原始研究引用。
- 不要把相关性或神经影像关联写成因果。
- 不要用“近年来，随着……”等空泛开头。
- 每段必须有明确论点、证据和限制。
- 每个实证、理论、方法、定义或机制判断必须能回到文献来源。
...
综述正文写完后，必须调用 `subskills/source-alignment/SKILL.md`，生成“综述原文—参考文献原文对应矩阵”。矩阵需要精确到：综述第几段、第几个实质性判断、引用了哪篇文献、对应文献的哪一页/哪一节/哪一段/哪一个表格或结果、支持强度如何、是否存在过度推断。

在最终回复中，请给我：运行模式、完成了哪些阶段和文件、哪些结论证据强、哪些结论证据弱、哪些引用或段落还需要人工核查、哪些阶段门控没有通过，以及是否可以称为 systematic/scoping review。
...
1. Identify 2–3 plausible journals from manuscript scope, article type, audience, methods, and constraints. Verify current official aims, article type, limits, checklist, data policy, fees, and AI policy; record access dates.
2. Write a journal-fit memo with reasons, risks, and a recommended target. Do not use impact factor alone.
3. Run editorial triage: scope, novelty, ethics, completeness, reporting, readability, and fatal defects.
4. Conduct independent review passes before synthesis:
   - domain/theory and literature;
   - methods, statistics, psychometrics, longitudinal inference, and reproducibility;
   - reporting, ethics, citations, privacy, and integrity;
   - devil's-advocate alternative explanations and strongest rejection case.
5. Produce one simulated decision: accept, minor revision, major revision, or reject. Make it consistent with listed defects.
6. Convert comments into `10_review/revision_matrix.csv`; assign severity, evidence, action, location, and status.
7. Revise the manuscript only from resolved evidence. Draft a point-by-point response that states what changed and where; justify declined requests respectfully.
8. Re-review changed sections and run final citation, numerical, methods, reporting, journal-format, ethics, and reproducibility audits.
